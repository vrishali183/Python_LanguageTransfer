...
firebase = firebase.FirebaseApplication(" Add firebase link", None)
...
def login(): 
    ...
    #add inside md5 password.encode()
    encrypted_password = hashlib.md5() 
    #assign encrypted_password.hexdigest() to hexadecimal password
    hexadecimal_password =  
...
#add register in command 
btn_reg = Button(registration_window, text="Sign Up" , font = 'arial 13 bold' , bg="#2D1E2F", fg="white", command=, relief=FLAT, padx=10)
btn_reg.place(relx=0.5,rely=0.75, anchor=CENTER)
#add login_window
btn_login_window = Button(registration_window, text="Log In" , font = 'arial 10 bold' , bg="#2D1E2F", fg="white", command=, relief=FLAT)
